...
input_features = []
for i in range(54):
    input_features.append(st.slider(f"Question {i+1}", 0, 4, 2))
if st.button("Predict"):
    input_data = np.array(input_features).reshape(1, -1)
    scaled_input = scaler.transform(input_data)
    prediction = model_rf.predict(input_data)[0]
    probability = model_rf.predict_proba(input_data)[0]
    st.header("Prediction:")
    if prediction == 0:
        st.write("The model predicts **no divorce**.")
        st.write(f"Probability of no divorce: {probability[0]:.2f}")
    else:
        st.write("The model predicts **divorce**.")
        st.write(f"Probability of divorce: {probability[1]:.2f}")
st.title("Context: Each Question in the Dataset")
questions = [
    ...
    "Q11 I think that one day in the future, when I look back, I see that my spouse and I have been in harmony.",
    ...
    "Q22 I know how my spouse wants to be taken care of when she/he is sick.",
    ...
    "Q38 I hate my spouse's way of opening a subject.",
